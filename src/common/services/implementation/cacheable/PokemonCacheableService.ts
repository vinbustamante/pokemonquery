...
@injectable()
export class PokemonCacheableService extends PokemonService
  ...
  getRecord(value: string): Promise<PokemonDto> {
    ...
    return this._fileCacheService.get(fileName).catch(err => {
      if (
        err instanceof CacheMissedException ||
        err instanceof CacheExpiredException
      ) {
        return super.getRecord(value).then(pokemon => {
