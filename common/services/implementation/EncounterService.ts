...
export class EncounterService implements IEncounterService {
    ...
    getById(id: string): Promise<EncounterDto[]> {
        return this._encounterRepository.getById(id)
            ...
            .then(records => {
                ...
                return this._flowService.each(records, record => {
                    return this._toMethods(record)
                        .then(methods => {
    ...
    private async _toMethods(record): Promise<string[]> {
        ...
        if (this._reflectionService.isObject(record) && record.locationId) {
            const response = await this._locationAreaRepository.getById(record.locationId);
            if (this._reflectionService.isObject(response) && this._reflectionService.isArray(response.encounter_method_rates)) {
                response.encounter_method_rates
                    .forEach(encounter => {
